...
const ResizableColumns = (props: Props) => {
    ...
    const [position, handleRef, containerRef, dragging] = useResizeHandle<HTMLDivElement, HTMLDivElement>(window.innerWidth * 0.7, 50, 5)
    ...
    useEffect(() => {
        ...
        setRightBorder(containerRef.current?.getBoundingClientRect().right ?? window.innerWidth)
...
const OverlaidViews: React.FC<Props> = (props) => {
    ...
    return (
        <>
            <div style={{position: "relative", height: "100%", display: "flex", width: "100%",justifyContent:"center", minHeight:"calc(100vh - 64px)",alignItems:"center",backgroundColor:"#eeeeee"}}>
                <Button style={{padding:"1rem",position: "absolute", top: 0, right: 0, zIndex: 1}}
                        onClick={() => setToggle(!toggle)}>{toggle ? "Preview" : "Eingabe"}
                </Button>
