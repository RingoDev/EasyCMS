...
import {Document, Page, StyleSheet, Text, View, Svg, G, Path} from "@react-pdf/renderer";
                ...
                <Page size="A4" style={styles.page}>
                    <View>
                        ...
                        <View style={styles.address}>
                            <View>
                                <View>
                                    <Text style={styles.smallAddress}>Gasthof Stöffelbauer - Linzerstraße 17 - 3350
                                        Haag</Text>
                                </View>
                                <View>
                                    {props.debtor.split("\n").map((a,index) => (
                        ...
                        <View style={styles.meta}>
                            ...
                            <View style={styles.metaRow}>
                                ...
                                <View style={{...styles.metaCell, ...styles.right}}>
                                    <Text style={tableStyles.text}> {dateToString(props.date, true, ".")}</Text>
                                </View>
                            </View>
                        </View>
                    </View>
...
const Table: React.FC<{ entries: Entry[] }> = (props) => {
    ...
    const sumNoTax = Math.round(props.entries.map(e => e.brutto * e.amount).reduce((p1, p2) => p1 + p2, 0) * 100) / 100


    const sumVat = Math.round(props.entries.map(e => e.brutto * e.amount * (e.vat / 100)).reduce((p1, p2) => p1 + p2, 0) * 100) / 100
        ...
        <>
            <View style={tableStyles.table}>
                ...
                {props.entries.map((e, index) => (
                    <View key={index} style={tableStyles.tableRow}>
                        <View style={tableStyles.tableCell}>
                            <Text style={tableStyles.text}>{dateToString(e.date, true, ".")}</Text>
                        </View>
                        <View style={{...tableStyles.tableCell, ...tableStyles.description}}>
                        ...
                        <View style={{...tableStyles.tableCell, ...tableStyles.result}}>
                            <Text
                                style={tableStyles.text}>{euroPad(Math.round((e.brutto * e.amount) * 100) / 100)}</Text>
                        </View>
                    </View>
                ))}
            </View>
            ...
            <View style={tableStyles.summary} wrap={false}>
                <View style={{...tableStyles.summaryRow}}>
                    <View style={{...tableStyles.summaryTableCell, ...tableStyles.left}}>
                        <Text style={tableStyles.text}>Summe Netto</Text>
                    </View>
                    <View style={{...tableStyles.summaryTableCell, ...tableStyles.right}}>
            ...
            <Svg style={logoStyles.logo} viewBox={"0 0 594 180"}>
                <G transform="translate(-1.5 -186.48)" stroke={"none"} fill={"none"}>
                    ...
                    <G strokeWidth={"2"} fill={"#015125"} stroke={"#015125"}>
                        ...
                        <Path
                            d="M393.6,364.46l19.72-88.29h10.54l19.71,88.29H430.93l-3.72-19H410l-3.72,19Zm31.25-30.88-6.2-32h-.25l-6.2,32Z"/>
                        <Path
                            d="M490.69,276.17v68.45a20,20,0,0,1-5.95,14.51,21,21,0,0,1-6.57,4.46,19.68,19.68,0,0,1-7.94,1.62,19.21,19.21,0,0,1-7.87-1.62,21.48,21.48,0,0,1-11-11,19.52,19.52,0,0,1-1.62-8V276.17h12.65v67.21c0,3.14.75,5.46,2.23,6.95a8.1,8.1,0,0,0,11.16,0q2.24-2.24,2.23-6.95V276.17Z"/>
                        <Path d="M502.6,364.46V276.17h37.69v11.91h-25V314h21.83V325.9H515.24v25.91h25v12.65Z"/>
                        <Path
                            d="M548.48,364.46V276.17h20.33q22.32,0,22.32,25.92a32.54,32.54,0,0,1-2.42,13.27,18.71,18.71,0,0,1-8.49,8.8l13.64,40.3H580.47l-11.78-37.7h-7.57v37.7Zm12.64-76.38V315.6h7.2a12.71,12.71,0,0,0,5.33-.93,7.27,7.27,0,0,0,3.1-2.66,12.18,12.18,0,0,0,1.36-4.28,47.07,47.07,0,0,0,0-11.78,11.74,11.74,0,0,0-1.49-4.4q-2.36-3.46-8.92-3.47Z"/>
                    </G>
