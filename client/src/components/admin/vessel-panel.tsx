...
import {Card, FilledInput, FormControl, Grid, InputLabel, MenuItem, Select} from "@material-ui/core";
...
const VesselPanel: React.FC<VesselPanelProps> = (props) => {
    ...
    return (
        <>
            ...
            <div>{props.vessel.children.map((c, index) =>
                <div key={index}>
                    <div>{c.name}</div>
                    <div>{c.type}</div>
                ...
                <Grid container>
                    <Grid item xs={6}>
                        <FormControl>
                            ...
                            <FilledInput value={name} onChange={e => setName(e.target.value)}/>
                        </FormControl>
                    </Grid>
                    <Grid item xs={6}>
                        <FormControl>
                            ...
                            <Select value={type} onChange={e => {
                                ...
                                <MenuItem value={ComponentTypes.Container}>{ComponentTypes.Container}
                                </MenuItem>
                                <MenuItem value={ComponentTypes.NumberInput}>{ComponentTypes.NumberInput}</MenuItem>
                                <MenuItem
                                    value={ComponentTypes.SinglelineText}>{ComponentTypes.SinglelineText}</MenuItem>
                                <MenuItem value={ComponentTypes.MultilineText}>{ComponentTypes.MultilineText}</MenuItem>
                                <MenuItem value={ComponentTypes.Checkbox}>{ComponentTypes.Checkbox}</MenuItem>
                            </Select>
                        </FormControl>
